...
export default function ClientNavMenu({ userRole, userName }: Props) {
  ...
  return (
    <>
      ...
      <div className="px-3 py-3 border-t border-gray-100 bg-white relative">
        <div className="flex items-center justify-between">
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium text-gray-700 truncate">{userName}</p>
            <p className="text-[10px] text-gray-400 font-bold uppercase mt-0.5">{userRole}</p>
          </div>
          <button
                  ...
                  <div className="divide-y divide-gray-50">
                    {notifications.map((n) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 hover:bg-gray-50 transition-colors ${!n.read ? 'bg-blue-50/40' : ''}`}
                      >
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
                  <div className="bg-white border border-slate-100 rounded-xl p-4 shadow-sm flex flex-col justify-between min-h-[110px]">
                    ...
                    <span className="text-2xl font-black text-slate-800 my-1">{overviewData?.recentFix?.poleCode ?? '—'}</span>
                  ...
                  <div className="bg-white border border-slate-100 rounded-xl p-4 shadow-sm flex flex-col justify-between min-h-[110px]">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Open Fault Reports</span>
                    <span className="text-3xl font-black text-slate-800 my-1">{overviewData?.openFaultsCount ?? '—'}</span>
                  ...
                  <div className="bg-white border border-slate-100 rounded-xl p-4 shadow-sm flex flex-col justify-between min-h-[110px]">
                    ...
                    <span className="text-3xl font-black text-slate-800 my-1">{overviewData?.ongoingMaintenanceCount ?? '—'}</span>
                  ...
                  <div className="bg-white border border-slate-100 rounded-xl p-4 shadow-sm flex flex-col justify-between min-h-[110px]">
                    ...
                    <span className="text-xl font-bold text-slate-800 truncate my-1">{overviewData?.leastLights?.barangay ?? '—'}</span>
                    <span className="text-[10px] text-slate-500 font-medium inline-flex items-center gap-1"><Lightbulb className="w-3 h-3" /> Total: {overviewData?.leastLights?.count ?? 0} units</span>
                  </div>
                  <div className="bg-white border border-slate-100 rounded-xl p-4 shadow-sm flex flex-col justify-between min-h-[110px]">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Street With Most Lights</span>
                    <span className="text-xl font-bold text-slate-800 truncate my-1">{overviewData?.mostLights?.barangay ?? '—'}</span>
                    <span className="text-[10px] text-slate-500 font-medium inline-flex items-center gap-1"><Zap className="w-3 h-3" /> Total: {overviewData?.mostLights?.count ?? 0} units</span>
                  </div>
                </div>
                    ...
                    {overviewData && overviewData.recentLogs.length > 0 ? (
                      <div className="space-y-2">
                        {overviewData.recentLogs.map((log) => (
                          <div key={log.id} className="flex items-center justify-between text-xs py-1.5 px-2 bg-white rounded-lg border border-slate-100">
                            <div className="flex items-center gap-2">
                              <span className="font-mono font-medium text-slate-700">{log.poleCode}</span>
                      ...
                      <div className="text-center text-slate-400 text-sm py-12">No recent activity logs.</div>
                    )}
                  </div>
                </div>
              </>
