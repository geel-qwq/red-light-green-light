...
async function getSuperadminOrAdminData(): Promise<DashboardData['cards']> {
  try {
    ...
    console.error("getSuperadminOrAdminData error:", error)
    return []
  }
}
...
async function getTechnicianData(userId: string): Promise<DashboardData['cards']> {
  try {
    ...
    console.error("getTechnicianData error:", error)
    return []
  }
}
...
export async function getUserFloatingDashboardData(userId: string): Promise<UserFloatingDashboardData> {
  try {
    ...
    console.error("getUserFloatingDashboardData error:", error)
    return { stats: { total: 0, open: 0, resolved: 0 }, recentReports: [] }
  }
}
...
export async function getTechnicianFloatingDashboardData(userId: string): Promise<TechnicianFloatingDashboardData> {
  try {
    ...
    console.error("getTechnicianFloatingDashboardData error:", error)
    return { stats: { assigned: 0, inProgress: 0, resolvedThisWeek: 0 }, workOrders: [] }
  }
}
...
export async function getSuperAdminFloatingDashboardData(): Promise<SuperAdminFloatingDashboardData> {
  try {
    ...
    console.error("getSuperAdminFloatingDashboardData error:", error)
    return { stats: { totalUsers: 0, totalTechnicians: 0, totalAdmins: 0, pendingWorkOrders: 0, openFaults: 0, totalPoles: 0 } }
  }
}
