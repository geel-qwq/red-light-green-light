...
import { X, ClipboardList, Wrench, CheckCircle2, MapPin } from "lucide-react";
...
interface TechnicianFloatingDashboardProps {
  isOpen: boolean
  onClose: () => void
  stats: {
    assigned: number
...
export default function TechnicianFloatingDashboard({
  isOpen,
  onClose,
  stats,
  workOrders,
  userName,
}: TechnicianFloatingDashboardProps) {
      ...
      <div className="relative z-10 w-[90vw] max-w-3xl bg-white/95 backdrop-blur-md rounded-[24px] shadow-[0_20px_50px_rgba(0,0,0,0.3)] flex flex-col p-6 md:p-8 max-h-[85vh] overflow-y-auto">
        ...
        <div className="mb-6 pr-8">
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Hi {firstName}! Here&apos;s a quick summary of your assigned tasks.
          </p>
        </div>

        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-white border border-blue-100 rounded-xl p-4 flex flex-col gap-2">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Assigned to Me</span>
            <span className="text-3xl font-black text-blue-600">{stats.assigned}</span>
            <span className="text-[10px] text-slate-400 font-medium flex items-center gap-1">
              <ClipboardList className="w-3 h-3" /> Pending / awaiting start
            </span>
          </div>
          ...
          <div className="bg-white border border-amber-100 rounded-xl p-4 flex flex-col gap-2">
            ...
            <span className="text-[10px] text-amber-600 font-medium flex items-center gap-1">
              <Wrench className="w-3 h-3" /> Active work orders
            </span>
          </div>

          <div className="bg-white border border-green-100 rounded-xl p-4 flex flex-col gap-2">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Resolved This Week</span>
            <span className="text-3xl font-black text-green-600">{stats.resolvedThisWeek}</span>
            <span className="text-[10px] text-emerald-600 font-medium flex items-center gap-1">
              <CheckCircle2 className="w-3 h-3" /> Closed in last 7 days
            </span>
          </div>
        </div>
        ...
        <div>
          <h2 className="text-sm font-bold text-slate-800 mb-1">My Work Orders</h2>
          <p className="text-xs text-slate-400 mb-4">Your latest assigned tasks</p>

          {workOrders.length === 0 ? (
            <div className="border border-dashed border-slate-200 rounded-xl bg-slate-50 py-10 text-center">
              <p className="text-slate-400 text-sm">No work orders assigned to you yet.</p>
            </div>
          ) : (
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-400 truncate">
                        {faultLabels[wo.faultReport.faultType] ?? wo.faultReport.faultType}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 flex-shrink-0 ml-4">
                    <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full ${statusStyles[wo.status] ?? 'bg-gray-100 text-gray-500'}`}>
                      {wo.status.replace('_', ' ')}
                    </span>
                    <span className="text-xs text-gray-300 whitespace-nowrap">{timeAgo(wo.assignedAt)}</span>
                  </div>
                </div>
              ))}
            </div>
