...
export default function MyWorkOrders({ workOrders }: { workOrders: WorkOrder[] }) {
  return (
    <div className="bg-white rounded-xl border border-gray-100 p-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="text-base font-bold text-gray-900">My Work Orders</h2>
      ...
      {workOrders.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-gray-400 text-sm">No work orders assigned to you yet.</p>
        </div>
      ) : (
            ...
            <div key={wo.id} className="py-3.5 flex items-center justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-400 mt-0.5 truncate">
                  {faultLabels[wo.faultReport.faultType] ?? wo.faultReport.faultType} · {timeAgo(wo.assignedAt)}
                </p>
              </div>
              <span
